...
const defaultMale =
  "https://images.unsplash.com/photo-1566492031773-4f4e44671857?q=80&w=500&auto=format&fit=crop";
...
const fetchProfile = async () => {
  try {
    ...
    const interestsRes = await API.get("/profile/my-interests");
    const interestCount = Array.isArray(interestsRes.data)
      ? interestsRes.data.length
      : 0;
...
// ✅ Use global image resolver for full URL handling
const getProfileImage = () => {
  if (!profile || !profile.profileImages || profile.profileImages.length === 0) {
    return profile?.gender === "female" ? defaultFemale : defaultMale;
  }

  return getImageUrl(profile.profileImages[0]); 
};
            ...
            <div className="bg-white rounded-2xl sm:rounded-3xl shadow-lg sm:shadow-2xl shadow-blue-500/10 border border-gray-100 overflow-hidden">
              ...
              <div className="pt-12 sm:pt-14 lg:pt-16 p-4 sm:p-6 lg:p-8">
                ...
                <motion.button
                  ...
                  className="w-full mt-3 sm:mt-4 bg-white border-2 border-red-100 text-red-600 hover:bg-red-50 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-semibold flex items-center justify-center gap-2 sm:gap-3 transition-all duration-300 text-sm sm:text-base"
                >
                ...
                <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 mb-4 sm:mb-6 flex items-center gap-2 sm:gap-3">
                  <div className="bg-purple-100 p-1.5 sm:p-2 rounded-lg sm:rounded-xl">
                    <Briefcase
                      ...
                      size={18}
                      className="sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-purple-600"
                    />
                  </div>
                  ...
                  <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 mb-4 sm:mb-6 flex items-center gap-2 sm:gap-3">
                    <div className="bg-indigo-100 p-1.5 sm:p-2 rounded-lg sm:rounded-xl">
                      <Globe
                        ...
                        size={18}
                        className="sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-indigo-600"
                      />
                    </div>
                    About Me
                  </h3>
